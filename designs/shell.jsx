...
function TrafficLights({ theme }) {
  const ring = theme.isDark ? 'rgba(0,0,0,0.35)' : 'rgba(0,0,0,0.15)';
  const dot = (bg) => (
    <div style={{
      width: 12, height: 12, borderRadius: '50%', background: bg,
      border: `0.5px solid ${ring}`,
      boxShadow: theme.isDark ? 'inset 0 0.5px 0 rgba(255,255,255,0.15)' : 'inset 0 0.5px 0 rgba(255,255,255,0.4)',
    }}/>
  );
  return (
...
function App() {
  ...
  const handleCloseTab = (tid) => {
    ...
    setProjects(prev => prev.map(p => (
      p.id !== activeProjectId ? p : { ...p, tabs: remaining }
    )));
    if (tid === activeTabId) {
      // pick neighbor: prefer the tab that was to the left, else first remaining
      const idx = proj.tabs.findIndex(t => t.id === tid);
  ...
  const handleNewTab = () => {
    ...
    // Generate a unique "shell N" label
    const existing = new Set(proj.tabs.map(t => t.label));
    let label = 'shell';
    let n = 2;
    while (existing.has(label)) { label = `shell ${n++}`; }
    setProjects(prev => prev.map(p => (
      p.id !== activeProjectId ? p : { ...p, tabs: [...p.tabs, { id: newId, label, cmd: 'zsh', script: 'idleShell', running: false }] }
    )));
